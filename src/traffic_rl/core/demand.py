...
Arrival times are drawn ONCE at world build (per key, per profile segment,
exponential inter-arrivals) from the ``demand`` stream. Piecewise-constant
profiles are exact — each segment draws its own homogeneous process.
...
Keys are caller-chosen: vehicle schedules draw per boundary ORIGIN, pedestrian
schedules per CROSSWALK (each crosswalk keyed by its leg name, so one per-leg
rate yields an independent stream at every intersection). For the phase-1
four-way world both key lists equal APPROACHES in the same order, so the draw
sequence — and therefore every golden trace — is unchanged.
...
def build_arrival_schedule(
    ...
    """Arrival times per key (caller's order), each sorted ascending.

    Iteration order is fixed (key-major, then segments) so a given stream
